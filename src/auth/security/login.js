...
export const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if(!email) {
            ...
            try{
                setShowloder(true)
                const response = await axios.post(LOGIN_URL, userLogins)
                if(response){
                    setShowModel(true)
    ...
    useEffect(() => {
        ...
        if(message === 'Login successfully'){
            ...
            const handleRedirect = setTimeout(() => {
                ...
                if(role !== null){
                    if(role.startsWith('7')){
                        navigate('/student_dashboard')
                    }else if(role.startsWith('9')){
                        navigate('/lecturer_dashboard')
                    }
                }
            }, 3000)
                        ...
                        <div className='d-flex align-items-start col-lg-6 col-12 gap-2 rounded-2 shadow p-3 sub-form-holder'>
                            <div className='d-flex align-items-center flex-column gap-3 col-12 inner-form-holder'>
                                ...
                                <form method='post' autoComplete='off' onSubmit={handleSubmit} className='d-flex flex-column align-items-start align-self-stretch gap-4 main-form'>
                                    ...
                                    <div className='position-relative d-flex flex-column justify-content-center align-items-center align-self-stretch button-holder'>
                                        {showloder ? <Loader /> : <button type='submit' id='btn'>
